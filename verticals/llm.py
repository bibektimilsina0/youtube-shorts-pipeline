...
def _call_gemini(prompt: str, max_tokens: int) -> str:
    ...
    url = (
        ...
        f"/models/{os.environ.get('GEMINI_TEXT_MODEL') or 'gemini-3.6-flash'}:generateContent"
    )
    # Gemini 3.x are thinking models: internal reasoning is billed against
    # maxOutputTokens (see usageMetadata.thoughtsTokenCount) before any text
    # is emitted. The caller's 1500 is sized for a non-thinking model and gets
    # exhausted mid-JSON, surfacing as a JSONDecodeError downstream. These
    # models reject thinkingConfig.thinkingBudget=0, so the fix is headroom:
    # ask for enough budget to cover the thoughts plus the full JSON payload.
    body = {
